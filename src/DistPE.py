...
"""
ABOUT: DENSE DISTANCE PRESERVING ENCODER
"""
...
class DistPE(AEBase):
    ...
    def __init__(self,
        ...
        if latent_dim is None:
            latent_dim = int(tf.math.ceil(tf.math.sqrt(tf.convert_to_tensor(self.input_size, dtype='float32'))))
    ...
    def fit(self, X, distance_matrix, epochs, batch_size=None):
        ...
        Data = tf.data.Dataset.from_generator(generator,
                                              ...
                                              ((tf.TensorShape([None,self.input_size]), tf.TensorShape([batch_size,batch_size])), (tf.TensorShape([batch_size,batch_size]))),
...
if __name__ == '__main__':
    ...
    # MODEL
    model = DistPE(input_size=X_train_pos.shape[1],
                   n_layers=3,
                   latent_dim=2,
                   optimizer='Adam',
                   activation='tanh',

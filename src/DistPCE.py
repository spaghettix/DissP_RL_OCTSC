...
"""
ABOUT: DENSE DISTANCE PRESERVING CONVOLUTIONAL ENCODER
"""
...
class DistPCE(AEBase):
    ...
    def fit(self, X, distance_matrix, epochs, batch_size=None):
        ...
        Data = tf.data.Dataset.from_generator(generator,
                                              ...
                                              ((tf.TensorShape([None,self.input_size,1]), tf.TensorShape([batch_size,batch_size])), (tf.TensorShape([batch_size,batch_size]))),
...
if __name__ == '__main__':
    ...
    # MODEL
    model = DistPCE(input_size=X_train_pos.shape[1],
                    n_layers=3,
                    latent_dim=2,
                    optimizer='Adam',
                    activation='tanh',
